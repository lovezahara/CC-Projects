...
public class DFAGen implements StateMachineGenI<Character> {
    ...
    public StateI<Character> read(InputStream in) throws InvalidStateMachineException, IOException {
        ...
        for (String[] arr: lines.keySet()) {
            ...
            //Checking for parsed statemachine.State info.
            if (arr[0].equals("s")) {
                if (arr.length < 2) {
            ...
            //Checking for parsed statemachine.Transition info.
       ...
       // if(startState == null) {throw new exceptions.InvalidStateMachineException("No start state found before EOF");}
        return states.get(startState.getLabel());
    }

    /**Method takes in a String, a boolean, and an int. Uses parameters to creates a statemachine.State Object instance and adds it to the
     * "states" HashMap with the statemachine.State's label as the key.
     * @param stateLbl - String representing the label of the statemachine.State Object instantiated in the method
    ...
    /**Method takes in a String[] that represents the parsed statemachine.State information read from the input stream/file.
     ...
     * @param arr - String[] representing parsed statemachine.State information
     * @param ln - represents line number of parsed state information, used in case of exception
    ...
    /**  Method takes in a boolean and a String. It checks whether the "states" HashMap already contains the start statemachine.State
     * and if not it creates a new statemachine.State Object that represent the start statemachine.State. Then adds it to "states" , unless the start
     * statemachine.State is already in "states" in which case it throws an exception.
    ...
    /** Method takes in a String[] that represents the parsed statemachine.Transition information read from the input stream/file.
     ...
     * @param arr - String[] representing parsed statemachine.Transition information
